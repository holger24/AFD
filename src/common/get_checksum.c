 ...
 **
 ** SYNOPSIS
 **   unsigned int get_checksum(const unsigned int icrc,
 **                             char               *mem,
 **                             int                size)
 ...
 **
 ** DESCRIPTION
 **   Gets the CRC-32 2 checksum (used by Zip, GZip, LZX, RAR, Arj).
 **   This function is taken from Dirk Stoecker <stoecker@epost.de>
 **   GetCRC.c program.
 **
 ** RETURN VALUES
 ...
 **   23.11.2012 H.Kiehl Adapted parameters of get_checksum() so it fits
 **                      better to the newer CRC-32 algorithm with
 **                      hardware support.
 **
 */
DESCR__E_M3
...
/*########################## get_checksum() #############################*/
unsigned int
get_checksum(const unsigned int icrc, char *mem, int size)
{
