...
/*
 ** NAME
 **   sf_sftp - send files via SFTP
 **
 ** SYNOPSIS
 **   sf_sftp <work dir> <job no.> <FSA id> <FSA pos> <msg name> [options]
 **
 **   options
 ...
 **
 ** DESCRIPTION
 **   sf_sftp sends the given files to the defined recipient via SFTP
 **   It does so by using it's own SFTP-client using the ssh program.
 **
...
main(int argc, char *argv[])
{
   ...
   do
   {
...
#endif
            fsa->job_status[(int)db.job_no].connect_status = SFTP_ACTIVE;
      ...
      if ((files_to_delete > 0) && (del_file_name_buffer != NULL))
      {
         ...
         {
            if ((status = sftp_dele(p_del_file_name)) != SUCCESS)
            {
...
# endif
         ...
         if (db.special_flag & SEQUENCE_LOCKING)
         {
            ...
            if ((db.retries > 0) && ((db.special_flag & UNIQUE_LOCKING) == 0))
            {
               (void)snprintf(p_end,
                              MAX_RECIPIENT_LENGTH + MAX_FILENAME_LENGTH - (p_end - initial_filename),
                              "-%u", db.retries - 1);
               if ((status = sftp_dele(initial_filename)) != SUCCESS)
               {
                  trans_log(DEBUG_SIGN, __FILE__, __LINE__, NULL, msg_str,
             ...
             (*p_file_size_buffer == 0))
         {
            ...
            if ((db.special_flag & SEND_ZERO_SIZE) == 0)
            {
               ...
               } while (bytes_buffered == (blocksize - buffer_offset));
            ...
            else
            {
               ...
               {
                  trans_db_log(INFO_SIGN, __FILE__, __LINE__, NULL,
                               "Closed file `%s'.", initial_filename);
               }
            }
            ...
            if (fsa->debug > NORMAL_MODE)
            {
               ...
               else
               {
                  trans_db_log(INFO_SIGN, __FILE__, __LINE__, NULL,
#if SIZEOF_OFF_T == 4
                               "Local file size of `%s' is %ld, remote size is %ld.",
#else
         ...
         {
            if ((status = sftp_set_file_time(initial_filename,
                                             *p_file_mtime_buffer,
                                             time(NULL))) != SUCCESS)
            {
               trans_log(WARN_SIGN, __FILE__, __LINE__, NULL, msg_str,
               ...
               for (k = 0; k < db.no_of_rhardlinks; k++)
               {
                  ...
                  else
                  {
                     ...
                     if (db.output_log == YES)
                     {
                        ...
                        (void)strcpy(hardlink_name[k], name);
                        hardlinks_done++;
                     }
#endif
            ...
            for (k = 0; k < db.no_of_rsymlinks; k++)
            {
               ...
               else
               {
                  ...
                  if (db.output_log == YES)
                  {
                     ...
                     (void)strcpy(softlink_name[k], to_name);
                     softlinks_done++;
                  }
#endif
...
# endif
      ...
      {
         if ((status = sftp_dele(db.lock_file_name)) != SUCCESS)
         {
            trans_log(ERROR_SIGN, __FILE__, __LINE__, NULL, msg_str,
...
sf_sftp_exit(void)
{
   if ((fsa != NULL) && (db.fsa_pos >= 0) && (fsa_pos_save == YES))
   {
      ...
      if ((diff_file_size_done > 0) || (diff_no_of_files_done > 0))
      {
         ...
         {
            if ((length + 10) <= (MAX_INT_LENGTH + 5 + MAX_OFF_T_LENGTH + 16 + MAX_INT_LENGTH + 21 + MAX_INT_LENGTH + 11 + MAX_INT_LENGTH + 1))
            {
               /* Write " [APPEND]" */
         ...
         {
            if ((length + 9) <= (MAX_INT_LENGTH + 5 + MAX_OFF_T_LENGTH + 16 + MAX_INT_LENGTH + 21 + MAX_INT_LENGTH + 11 + MAX_INT_LENGTH + 1))
            {
               /* Write " [BURST]" */
               buffer[length] = ' '; buffer[length + 1] = '[';
