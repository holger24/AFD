 ...
 **   05.09.2025 H.Kiehl Added Version 9.
 **
 */
DESCR__E_M1
...
#ifdef NEW_FRA
/* Version 9 */
#define MAX_DIR_ALIAS_LENGTH_9     MAX_DIR_ALIAS_LENGTH     /*  10 */
#define MAX_HOSTNAME_LENGTH_9      MAX_HOSTNAME_LENGTH      /*   8 */
#define MAX_RECIPIENT_LENGTH_9     MAX_RECIPIENT_LENGTH     /* 256 */
#define AFD_WORD_OFFSET_9          (SIZEOF_INT + 4 + SIZEOF_INT + 4)
#define MAX_FRA_TIME_ENTRIES_9     MAX_FRA_TIME_ENTRIES     /*  12 */
#define MAX_WAIT_FOR_LENGTH_9      MAX_WAIT_FOR_LENGTH      /*  64 */
#define MAX_TIMEZONE_LENGTH_9      MAX_TIMEZONE_LENGTH      /*  32 */
#define MAX_REMOVE_ACTION_LENGTH_9 MAX_REMOVE_ACTION_LENGTH /*  64 */
#define MAX_FILENAME_LENGTH_9      MAX_FILENAME_LENGTH      /* 256 */
struct bd_time_entry_9
       {
#ifdef HAVE_LONG_LONG
          unsigned long long continuous_minute;
          unsigned long long minute;
...
struct fileretrieve_status_9
       {
          char                   dir_alias[MAX_DIR_ALIAS_LENGTH_9 + 1];
          char                   host_alias[MAX_HOSTNAME_LENGTH_9 + 1];
          char                   url[MAX_RECIPIENT_LENGTH_9];
          char                   ls_data_alias[MAX_DIR_ALIAS_LENGTH_9 + 1];
          char                   retrieve_work_dir[MAX_FILENAME_LENGTH_9];
          char                   wait_for_filename[MAX_WAIT_FOR_LENGTH_9];
          char                   timezone[MAX_TIMEZONE_LENGTH_9 + 1];
          char                   remove_action[MAX_REMOVE_ACTION_LENGTH_9]; /* New */
          struct bd_time_entry_9 te[MAX_FRA_TIME_ENTRIES_9];
          struct bd_time_entry_9 ate;
          unsigned char          dir_status;
          unsigned char          remove;
          unsigned char          stupid_mode;
          unsigned char          delete_files_flag;
...
#ifdef NEW_FRA
   else if ((old_version == 0) && (new_version == 9))
        {
           struct fileretrieve_status_0 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
           ...
           new_size = old_no_of_dirs * sizeof(struct fileretrieve_status_9);
           if ((ptr = malloc(new_size)) == NULL)
           {
              system_log(ERROR_SIGN, __FILE__, __LINE__,
                         "malloc() error [%d %d] : %s",
...
# endif
           ...
           new_fra = (struct fileretrieve_status_9 *)ptr;
           ...
           for (i = 0; i < old_no_of_dirs; i++)
           {
              ...
              new_fra[i].wait_for_filename[0] = '\0';
              new_fra[i].timezone[0] = '\0';
              new_fra[i].remove_action[0] = '\0';
              (void)memset(&new_fra[i].te[0], 0,
                           (MAX_FRA_TIME_ENTRIES_9 * sizeof(struct bd_time_entry_9)));
              new_fra[i].dir_status             = old_fra[i].dir_status;
              new_fra[i].remove                 = old_fra[i].remove;
              new_fra[i].stupid_mode            = old_fra[i].stupid_mode;
              new_fra[i].protocol               = old_fra[i].protocol;
              ...
              new_fra[i].accumulate_size        = 0;
              new_fra[i].locked_file_time       = -1;
              (void)memset(&new_fra[i].ate, 0, sizeof(struct bd_time_entry_9));
              new_fra[i].max_errors             = 0;
              new_fra[i].error_counter          = 0;
           }
           ptr = (char *)old_fra;
            ...
            */
           if ((ptr = mmap_resize(old_fra_fd, ptr,
                                  new_size + AFD_WORD_OFFSET_9)) == (caddr_t) -1)
           {
              system_log(ERROR_SIGN, __FILE__, __LINE__,
                         "Failed to mmap_resize() %s : %s",
                         old_fra_stat, strerror(errno));
           ...
           *(ptr + SIZEOF_INT + 4 + SIZEOF_INT + 2) = 0;        /* Not used. */
           *(ptr + SIZEOF_INT + 4 + SIZEOF_INT + 3) = 0;        /* Not used. */
           *old_fra_size = new_size + AFD_WORD_OFFSET_9;

           system_log(DEBUG_SIGN, NULL, 0,
                      "Converted FRA from verion %d to %d.",
                      (int)old_version, (int)new_version);
        }
   else if ((old_version == 1) && (new_version == 9))
        {
           struct fileretrieve_status_1 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 2) && (new_version == 9))
        {
           struct fileretrieve_status_2 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 3) && (new_version == 9))
        {
           struct fileretrieve_status_3 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 4) && (new_version == 9))
        {
           struct fileretrieve_status_4 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 5) && (new_version == 9))
        {
           struct fileretrieve_status_5 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 6) && (new_version == 9))
        {
           struct fileretrieve_status_6 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
   ...
   else if ((old_version == 7) && (new_version == 9))
        {
           struct fileretrieve_status_7 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
            ...
            * number.
            */
           if ((ptr = mmap_resize(old_fra_fd, ptr, new_size + AFD_WORD_OFFSET_9)) == (caddr_t) -1)
           {
              system_log(ERROR_SIGN, __FILE__, __LINE__,
                         "Failed to mmap_resize() %s : %s",
                         old_fra_stat, strerror(errno));
   ...
   else if ((old_version == 8) && (new_version == 9))
        {
           struct fileretrieve_status_8 *old_fra;
           struct fileretrieve_status_9 *new_fra;
...
# endif
           ...
           old_features = *(ptr + SIZEOF_INT + 1);
           ptr += AFD_WORD_OFFSET_8;
           old_fra = (struct fileretrieve_status_8 *)ptr;
...
# endif
           ...
           for (i = 0; i < old_no_of_dirs; i++)
           {
              ...
              new_fra[i].bytes_received         = old_fra[i].bytes_received;
              new_fra[i].files_received         = old_fra[i].files_received;
              new_fra[i].dir_options            = old_fra[i].dir_options;
              new_fra[i].last_retrieval         = old_fra[i].last_retrieval;
              new_fra[i].next_check_time        = old_fra[i].next_check_time;
              new_fra[i].unknown_file_time      = old_fra[i].unknown_file_time;
              new_fra[i].queued_file_time       = old_fra[i].queued_file_time;
