...
write_header(int fd)
{
   ...
   if (no_of_search_hosts > 0)
   {
      ...
      length += snprintf(&buffer[length], 1024 - length,
                         "\tHost name     : %s", search_recipient[0]);
      if (length >= 1024)
      {
         length = 1024;
         goto write_data;
   ...
   if (toggles_set & SHOW_UNSENT_INPUT)
   {
      ...
      {
         length += snprintf(&buffer[length], 1024 - length,
                            "\tQueue typ     : UNSENT INPUT");
      }
      else
      {
         length += snprintf(&buffer[length], 1024 - length, ", UNSENT INPUT");
      }
      if (length >= 1024)
      {
         length = 1024;
   ...
   if (toggles_set & SHOW_UNSENT_OUTPUT)
   {
      ...
      {
         length += snprintf(&buffer[length], 1024 - length,
                            "\tQueue typ     : UNSENT OUTPUT");
      }
      else
      {
         length += snprintf(&buffer[length], 1024 - length, ", UNSENT OUTPUT");
      }
      if (length >= 1024)
      {
         length = 1024;
   ...
   if (toggles_set & SHOW_RETRIEVES)
   {
      ...
      {
         length += snprintf(&buffer[length], 1024 - length, ", RETRIEVES");
      }
      if (length >= 1024)
      {
         length = 1024;
   ...
   if (toggles_set & SHOW_TIME_JOBS)
   {
      ...
      {
         length += snprintf(&buffer[length], 1024 - length, ", TIME");
      }
      if (length >= 1024)
      {
         length = 1024;
