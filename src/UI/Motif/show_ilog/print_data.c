...
write_summary(int fd, char *sum_sep_line)
{
   ...
   length = snprintf(buffer, (2 * (MAX_OUTPUT_LINE_LENGTH + SHOW_LONG_FORMAT + 5 + 1)),
                     "%s\n%s\n", sum_sep_line, summary_str);
   if (length > (2 * (MAX_OUTPUT_LINE_LENGTH + SHOW_LONG_FORMAT + 5 + 1)))
   {
      length = 2 * (MAX_OUTPUT_LINE_LENGTH + SHOW_LONG_FORMAT + 5 + 1);
   }
