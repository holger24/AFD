/*
 *  afdds.c - Part of AFD, an automatic file distribution program.
...
/*
 ** NAME
 **   afdds - TLS TCP command daemon for the AFD
 **
 ** SYNOPSIS
 **   afdds [--version][-w <AFD working directory>]
 **
 ** DESCRIPTION
 **   This is a small TLS tcp command server at port AFD_PORT_NO that
 ...
 **
 ** HISTORY
 **   29.03.2022 H.Kiehl Forked from afdd.
 **   21.09.2022 H.Kiehl Allow user to bind to a specific network
 **                      interface.
 */
DESCR__E_M1
...
static int        in_child = NO,
                  max_afdds_connections = MAX_AFDD_CONNECTIONS,
                  new_sockfd,
                  sockfd,
                  no_of_connections;
static SSL_CTX    *ctx;
...
static void       afdds_exit(void),
                  configure_context(SSL_CTX *),
                  get_afdds_config_value(char *, char *, int *),
#ifdef LINUX
...
main(int argc, char *argv[])
{
   ...
   no_of_connections = 0;
   (void)strcpy(port_no, DEFAULT_AFD_TLS_PORT_NO);
   get_afdds_config_value(bind_address, port_no, &max_afdds_connections);
   if ((pid = malloc((max_afdds_connections * sizeof(pid_t)))) == NULL)
   {
      system_log(ERROR_SIGN, __FILE__, __LINE__,
                 _("Failed to malloc() %d bytes : %s"),
                 (max_afdds_connections * sizeof(pid_t)),
                 strerror(errno));
      exit(INCORRECT);
   }
   (void)memset(pid, 0, max_afdds_connections * sizeof(pid_t));
   ...
   if ((ptr = lock_proc(AFDDS_LOCK_ID, NO)) != NULL)
   {
      system_log(ERROR_SIGN, __FILE__, __LINE__,
                 _("Process AFDDS already started by %s"), ptr);
      (void)fprintf(stderr, _("Process AFDDS already started by %s : (%s %d)\n"),
...
# endif
   ...
   method = afd_encrypt_server_method();
   if ((ctx = SSL_CTX_new(method)) == NULL)
   {
      system_log(FATAL_SIGN, __FILE__, __LINE__,
                 _("Unable to create SSL context."));
      exit(INCORRECT);
   }

   /* Set certificate and key if they exist. */
   ...
   if (bind_address[0] == '\0')
   {
      system_log(INFO_SIGN, NULL, 0, _("Starting %s at port %d (%s)"),
                 AFDDS, port, PACKAGE_VERSION);
   }
   else
   {
      system_log(INFO_SIGN, NULL, 0, _("Starting %s at port %d on %s (%s)"),
                 AFDDS, port, bind_address, PACKAGE_VERSION);
   }
   ...
   for (;;)
   {
      ...
      if (FD_ISSET(sockfd, &rset))
      {
         ...
         if (number_of_trusted_ips > 0)
         {
            ...
            {
               system_log(DEBUG_SIGN, __FILE__, __LINE__,
                          _("AFDDS: Illegal access from %s"), remote_ip_str);
         ...
         if (no_of_connections >= max_afdds_connections)
         {
            system_log(DEBUG_SIGN, __FILE__, __LINE__,
                       _("AFDDS: Connection attempt from %s, but denied because max connection (%d) reached."),
                       remote_ip_str, max_afdds_connections);
         }
         else
         {
            system_log(DEBUG_SIGN, NULL, 0,
                       _("AFDDS: Connection from %s"), remote_ip_str);
         }
         ...
         if ((status = SSL_accept(ssl)) <= 0)
         {
            ...
            switch (status)
            {
               case SSL_ERROR_SYSCALL :
                  system_log(ERROR_SIGN, __FILE__, __LINE__,
                             _("SSL_accept() error (%d) : %s"),
                             status, strerror(errno));
                  break;

               default : /* Error */
                  system_log(ERROR_SIGN, __FILE__, __LINE__,
                             _("SSL_accept() error (%d)"), status);
                  break;
            }
            SSL_shutdown(ssl);
            SSL_free(ssl);
         ...
         else
         {
            if (no_of_connections >= max_afdds_connections)
            {
               ...
               if (ssl_write(ssl, reply, length) != length)
               {
                  system_log(WARN_SIGN, __FILE__, __LINE__,
            ...
            {
               if ((pos = get_free_connection(max_afdds_connections)) < 0)
               {
                  if (ssl_write(ssl, "421 Service not available.\r\n", 28) != 28)
                  {
                     system_log(WARN_SIGN, __FILE__, __LINE__,
                                "Failed to ssl_write() `421 Service not available' to socket: %s",
                                strerror(errno));
                  }
                  SSL_shutdown(ssl);
                  SSL_free(ssl);
               ...
               else
               {
                  switch (pid[pos] = fork())
                  {
                     ...
                     case 0  : /* Child process to serve user. */
                        in_child = YES;
                        (void)close(sockfd);
                        handle_request(ssl, new_sockfd, pos, trusted_ip_pos,
                                       remote_ip_str);
                        exit(0);
...
configure_context(SSL_CTX *ctx)
{
   ...
   (void)snprintf(filename, MAX_PATH_LENGTH, "%s%s/%s",
                  p_work_dir, ETC_DIR, AFDDS_CERT_FILENAME);
   if (eaccess(filename, R_OK) == 0)
   {
      int status;

      if ((status = SSL_CTX_use_certificate_chain_file(ctx, filename)) <= 0)
      {
      }
      else
      {
         (void)snprintf(filename, MAX_PATH_LENGTH, "%s%s/%s",
                        p_work_dir, ETC_DIR, AFDDS_KEY_FILENAME);
         if (eaccess(filename, R_OK) == 0)
         {
            if ((status = SSL_CTX_use_PrivateKey_file(ctx, filename,
                                                      SSL_FILETYPE_PEM)) <= 0)
            {
            }
         }
...
zombie_check(void)
{
   ...
   for (i = 0; i < max_afdds_connections; i++)
   {
      if ((pid[i] > 0) &&
...
/*+++++++++++++++++++++++ get_afdds_config_value() ++++++++++++++++++++++*/
static void                                                                
get_afdds_config_value(char *bind_address,
                       char *port_no,
                       int  *max_afdds_connections)
{
   char *buffer;
       ...
       (read_file_no_cr(afd_config_file, &buffer, YES, __FILE__, __LINE__) != INCORRECT))
   {
...
#ifdef HAVE_SETPRIORITY
      if (get_definition(buffer, AFDDS_PRIORITY_DEF,
      ...
      if (get_definition(buffer, MAX_AFDD_CONNECTIONS_DEF,
                         value, MAX_INT_LENGTH) != NULL)
      {
         *max_afdds_connections = atoi(value);
         if (*max_afdds_connections < 0)
         {
            system_log(DEBUG_SIGN, __FILE__, __LINE__,
                       _("Incorrect value (%d) set in AFD_CONFIG for %s. Setting to default %d."),
                       *max_afdds_connections, MAX_AFDD_CONNECTIONS_DEF,
                       MAX_AFDD_CONNECTIONS);
            *max_afdds_connections = MAX_AFDD_CONNECTIONS;
         }
      }

      if (get_definition(buffer, AFD_TLS_PORT_DEF, value,
                         MAX_IP_LENGTH + 1 + MAX_INT_LENGTH) != NULL)
      {
         ...
         if (*tmp_ptr == ':')
         {
            ...
            if (i == MAX_INT_LENGTH)
            {
               system_log(WARN_SIGN, __FILE__, __LINE__,
                          ...
                          MAX_INT_LENGTH, DEFAULT_AFD_TLS_PORT_NO);
               (void)strcpy(port_no, DEFAULT_AFD_TLS_PORT_NO);
            }
            else
            {
         ...
         if ((port < lower_limit) && (port > upper_limit))
         {
            system_log(WARN_SIGN, __FILE__, __LINE__,
                       ...
                       AFD_TLS_PORT_DEF, port, lower_limit, upper_limit);
         }
      }
      ...
      /*
       * Read all IP-numbers that may connect to AFDDS. If none is found
...
afdds_exit(void)
{
   if (in_child == NO)
   {
      ...
      if ((kill_list = malloc(max_afdds_connections * sizeof(pid_t))) == NULL)
      {
      ...
      system_log(INFO_SIGN, NULL, 0, _("Stopped %s."), AFDDS);
   }
